...
const MOCK_NOTES: NotePreview[] = [
  ...
  },
  {
    ...
    preview:
      "The 'To be or not to be' soliloquy reflects Hamlet's internal conflict between action and inaction...",
    folder: "English",
  },
  {
    id: "n3",
  ...
  if (size === "small") {
    return (
      <div className="card flex flex-col h-full">
        <h2 className="text-xs font-serif font-semibold text-charcoal mb-1.5 shrink-0">
          Recent Notes
        </h2>
        <div className="flex-1 space-y-0.5 overflow-y-auto min-h-0">
          {notes.map((n) => (
            <button
              ...
              className="w-full text-left flex items-center gap-1.5 px-1 py-1 rounded hover:bg-parchment/60 transition-colors"
            >
              <FileText size={11} className="text-muted shrink-0" />
              <span className="text-[11px] text-charcoal truncate">{n.title}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="flex-1 space-y-1.5 overflow-y-auto min-h-0">
          {notes.map((n) => (
            <button
              key={n.id}
              onClick={() => navigate(n.id)}
              className="w-full text-left bg-parchment/60 rounded-lg px-2.5 py-2 hover:bg-parchment transition-colors"
            >
              <div className="flex items-center justify-between mb-0.5">
                <span className="font-serif font-semibold text-xs text-charcoal truncate">
                  {n.title}
                </span>
              ...
              <p className="text-[10px] text-muted line-clamp-2">{n.preview}</p>
              {n.folder && (
                <span className="inline-block mt-0.5 text-[9px] text-muted bg-parchment-dark rounded px-1 py-0.5">
                  {n.folder}
                </span>
              )}
            </button>
      ...
      <div className="flex-1 space-y-1 overflow-y-auto min-h-0">
        {notes.map((n) => (
          <button
            key={n.id}
            onClick={() => navigate(n.id)}
            className="w-full text-left flex items-start gap-2 px-1.5 py-1.5 rounded-lg hover:bg-parchment/60 transition-colors"
          >
            <FileText size={13} className="text-muted shrink-0 mt-0.5" />
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between">
                <span className="font-serif text-xs font-semibold text-charcoal truncate">
                  {n.title}
                </span>
                <span className="text-[9px] text-muted shrink-0 ml-2">
                  {relativeTime(n.updatedAt)}
              ...
              <p className="text-[10px] text-muted truncate">{n.preview}</p>
            </div>
          </button>
        ))}
      </div>
